...
class AppStore extends Component {
  state = {
    stateAppList: appsList.filter(eachList => eachList.category === 'SOCIAL'),
    activeTab: 'SOCIAL',
    searchInput: '',
  }
  ...
  changeTab = Value => {
    ...
    const FilteredList = appsList.filter(
      eachList =>
        eachList.category === Value &&
        eachList.appName.toLowerCase().includes(searchInput),
    )
    console.log(FilteredList)
    this.setState({stateAppList: FilteredList, activeTab: Value})
  }

  searchFunction = event => {
    const {activeTab} = this.state
    console.log(event.target.value.toLowerCase())
    const SearchFilterList = appsList.filter(
      eachList =>
        eachList.appName
          .toLowerCase()
    ...
    this.setState({
      stateAppList: SearchFilterList,
      searchInput: event.target.value.toLowerCase(),
    })
  }

  render() {
    const {stateAppList, activeTab, searchInput} = this.state
    return (
